...
neural activations (plasticity) in (the somatosensory and motor) cortex does doen depend on 
whether or not the when BMI is inactive (subject is idle)
...
#    eegfilename = 'S9017_ses3_closeloop_block0000-epo.fif'
        ...
        # Outlier detection
        A = MRCP.idOutliers(data.reshape(56,-1), dim=1,
...
for ep in eeg:    
    print(ep)    
    evoked = ep.average().detrend(order=1)
    evoked.plot();
    ev.append(evoked)
...
for source in stcs:    
    cond1.append(copy.deepcopy(source).crop(-0.8, 0))
    cond2.append(copy.deepcopy(source).crop(0, 0.8))

#%%
n_vertices_sample, n_times = cond2[2].data.shape
